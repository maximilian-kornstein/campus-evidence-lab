# ED Certification Batch 015 Review

`data/ed-certification-batch-015-review.json` is the fifteenth bounded ED Campus Safety dataset source-to-record certification review.
...
Batch 015 was initialized from the next ED dataset work window exposed by the certification-batch manifest after Batch 014 was applied. The review artifact freezes those 250 event ids so regeneration cannot silently move the review wave to later records.

## Blocked Records

The blocked records are not certified because the source-cell locator remains ambiguous:

- `evt_2026_2829`: multiple workbook rows matched `cape_cod_community_college`, `VANDAL_SEX23`, and count `2`.
- `evt_2026_2881`: multiple workbook rows matched `coastline_community_college`, `INTIM_SEX22`, and count `1`.
...
Batch 015 is a named review artifact with its own certification basis: `ed_certification_batch_015_internal_source_to_record_review`.

Regenerating the pipeline must update this same reviewed record set. To review another ED batch, create a new named batch-review artifact rather than letting Batch 015 move.
