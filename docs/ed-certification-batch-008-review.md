# ED Certification Batch 008 Review

`data/ed-certification-batch-008-review.json` is the eighth bounded ED Campus Safety dataset source-to-record certification review.
...
Batch 008 was initialized from the next ED dataset work window exposed by the certification-batch manifest after Batch 007 was applied. The review artifact freezes those 250 event ids so regeneration cannot silently move the review wave to later records.

## Blocked Records

The blocked records are not certified because the source-cell locator remains ambiguous:

- `evt_2026_2277`: multiple workbook rows matched `pasadena_city_college`, `VANDAL_REL24`, and count `1`.
- `evt_2026_2290`: multiple workbook rows matched `state_university_of_new_york_at_plattsburgh`, `INTIM_RAC23`, and count `1`.

These records remain blocked until the locator ambiguity can be resolved without guessing.

## What This Certifies

For the 248 certified records, certification means:
...
Batch 008 is a named review artifact with its own certification basis: `ed_certification_batch_008_internal_source_to_record_review`.

Regenerating the pipeline must update this same reviewed record set. To review another ED batch, create a new named batch-review artifact rather than letting Batch 008 move.

Batch 009 now continues this sequence in `data/ed-certification-batch-009-review.json`.
