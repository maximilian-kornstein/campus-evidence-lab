# ED Certification Batch 013 Review

`data/ed-certification-batch-013-review.json` is the thirteenth bounded ED Campus Safety dataset source-to-record certification review.
...
Batch 013 was initialized from the next ED dataset work window exposed by the certification-batch manifest after Batch 012 was applied. The review artifact freezes those 250 event ids so regeneration cannot silently move the review wave to later records.

## Blocked Records

No Batch 013 records are blocked in this review artifact. If later source-to-record checks identify a locator, category, date, label, response-depth, rationale, or overclaim issue, the record must be moved to a visible unresolved status rather than kept certified.
...
Batch 013 is a named review artifact with its own certification basis: `ed_certification_batch_013_internal_source_to_record_review`.

Regenerating the pipeline must update this same reviewed record set. To review another ED batch, create a new named batch-review artifact rather than letting Batch 013 move.
