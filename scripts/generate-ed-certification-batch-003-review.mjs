...
const [events, certificationBatches, edDatasetProvenanceAudit, existingReview, manifest] = await Promise.all([
  ...
  existsSync(paths.edCertificationBatch003Review) ? readJson(paths.edCertificationBatch003Review) : null,
...
if (errors.length) {
  throw new Error(`ED certification Batch 003 review validation failed:\n${errors.map((error) => `- ${error}`).join("\n")}`);
}

await writeJson(paths.edCertificationBatch003Review, review);

console.log(
  `Generated ED certification Batch 003 review: ${review.totals.records} records, ${review.totals.certified} certified, ${review.totals.not_certified} not certified, ${review.totals.blocked} blocked.`
);
