...
if (!Number.isInteger(targetCount) || targetCount <= 0) {
  throw new Error("Usage: node scripts/import-ed-campus-safety-additional-cells.mjs <target-count> [workbook-path]");
}
...
for (const event of eventsData) {
  if (!event.source_ids?.includes(sourceId)) continue;
  const codeTag = (event.tags ?? []).find((tag) => workbookCodeTags.has(tag));
  if (!codeTag) continue;
  existingSourceKeys.add(`${event.school_id}|${event.date.slice(0, 4)}|${codeTag}`);
}
...
for (const row of rows.slice(1)) {
  ...
  for (const [index, header] of headers.entries()) {
    ...
    const key = `${schoolId}|${year}|${codeTag}`;
    if (existingSourceKeys.has(key) || seenCandidateKeys.has(key)) continue;
...
if (selected.length < batchSize) {
  throw new Error(`Only found ${selected.length} eligible additional source cells; need ${batchSize}.`);
}
...
for (const record of selected) {
  ...
  eventsData.push({
    ...
    confidence: "Medium",
    tags: ["ed-campus-safety-data", "clery", "hate-crime-statistics", "on-campus", record.codeTag],
    ...
    changelog: [
      {
        date: importedAt,
        note: `Imported from an additional Department of Education Campus Safety and Security 2025 Excel hate-crime data-file cell for the ${targetCount}-record expansion.`
      }
    ]
  });
}

const briefId = `brief_2026_06_03_ed_campus_safety_hate_data_${targetCount}_additional_cells`;
if (!briefsData.some((brief) => brief.id === briefId)) {
  briefsData.push({
    id: briefId,
    title: `Department of Education Campus Safety Hate-Crime Additional Cell Expansion ${targetCount}`,
    week_start: "2026-06-01",
    week_end: "2026-06-07",
    published_date: importedAt,
    summary: `Campus Evidence Lab added ${selected.length} additional distinct Department of Education Campus Safety and Security hate-crime data-file cells to reach the ${targetCount}-record checkpoint while preserving school, state, year, offense, and bias-code deduplication.`,
...
console.log(
  `Imported ${selected.length} additional Department of Education cells for target ${targetCount}: ` +
