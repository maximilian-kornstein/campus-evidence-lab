...
import { DISTRIBUTION_POLICY_VERSION, datasetSocialCopy } from "./social-copy.mjs";
...
function semanticFacts(rows) {
  ...
  for (const row of rows) {
    const fact = {
      year: row.event.date.slice(0, 4),
      category: row.event.category,
      affected_communities: [...new Set(row.event.affected_communities ?? [])].sort(),
      geography: geography(row.event),
    };
    const key = JSON.stringify(fact);
    const existing = grouped.get(key) ?? { ...fact, value: 0, supporting_record_ids: [] };
    existing.value += row.value;
    existing.supporting_record_ids.push(row.event.id);
    grouped.set(key, existing);
  }
  return [...grouped.values()].map((fact) => ({ ...fact, supporting_record_ids: [...new Set(fact.supporting_record_ids)].sort() }));
}
...
export function buildCertifiedDossiers({ events, schools, sources, certificationRows, siteUrl = "https://campusevidencelab.org", generatedAt = new Date().toISOString(), minimumCells = 4 }) {
  ...
  for (const [schoolId, cellMap] of grouped) {
    ...
    const facts = semanticFacts(rows);
    ...
    for (const angle of angles) {
      ...
      const angleFacts = angle.type === "latest_year" ? facts.filter((fact) => fact.year === latestYear) : facts;
      const socialCopy = datasetSocialCopy({ schoolName: school.name, year: latestYear, facts: angleFacts, canonicalUrl });
      signals.push({
        id: signalId,
