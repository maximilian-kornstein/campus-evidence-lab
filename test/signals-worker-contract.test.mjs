...
test("worker exposes guarded ingest, complaint pause, attribution, source redirect, metrics, and shadow-gated publishing", async () => {
  ...
  for (const route of ["/api/signals/ingest", "/api/distributions/withdraw", "/api/complaints", "/api/track", "/api/metrics", "/api/control", "/api/activate", "/api/providers", "/api/followers", "/api/replies", "/api/opt-out"]) assert.ok(worker.includes(route));
